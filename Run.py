...
#setup sweep, sweep config already set in wandb
def train(config=None):
    ...
    epoch_to_start_regressor=config['epoch_to_start_regressor']
    ...
    for epoch in range(num_epochs):
        ...
        wandb.log({"loss_val": loss_val.item(),'loss_R2':r2, "loss": total_loss})
    ...
    wandb.log({"Test R2":r2,"Test MAE":mae})

    #save data for plotting
    data = {'Y_test_np':Y_test_np,'predictions_np_test':predictions_np_test,'Y_train_np':Y_train_np,'predictions_np_train':predictions_np_train,'test r2':r2,'test mae':mae}
    np.save('predictions_np_test'+str(run.name)+'.npz',data)
...
"""    # Plotting for target A
    ...
    plt.scatter(Y_train_np[:, 0], predictions_np_train[:, 0], alpha=0.5, c='r')
    plt.scatter(Y_test_np[:, 0], predictions_np_test[:, 0], alpha=1,color='k')
    plt.plot([0,1], [0,1], 'k--', lw=2)
    plt.title('Target A: Actual vs. Predicted')
    plt.xlabel('Actual')
    plt.ylabel('Predicted')

    # Plotting for target B
    plt.subplot(1, 2, 2)  # 1 row, 2 columns, second subplot
    plt.scatter(Y_train_np[:, 1], predictions_np_train[:, 1], alpha=0.5, c='r')
    plt.scatter(Y_test_np[:, 1], predictions_np_test[:, 1], alpha=1,color='k')
    plt.plot([0,1], [0,1], 'k--', lw=2)
    plt.title('Target B: Actual vs. Predicted')
    plt.xlabel('Actual')
    plt.ylabel('Predicted')
    
    plt.tight_layout()
    plt.savefig('current_model'+str(run.name)+'.png')
    plt.close()"""


#agent

wandb.agent("wc34oq4u",train, count=100,project='NMR_Prediction_of_Chem_Props')
